...
router.patch('/requests/:requestId/items/:itemId/supply', async (req, res) => {
  try {
    ...
    if (typeof pricePerUnit !== 'number' || pricePerUnit <= 0) {
      return res.status(400).json({ message: 'Valid pricePerUnit is required' });
    }
    ...
    if (!item) return res.status(404).json({ message: 'Item not found in request' });
...
router.patch('/requests/:requestId/items/:itemId/reject-supply', async (req, res) => {
  try {
    const { requestId, itemId } = req.params;
...
router.post('/process-supply/:requestId/:itemId', async (req, res) => {
  ...
  try {
    ...
    if (item.status !== 'supplied') {
      return res.status(400).json({ message: 'Item has not been marked as supplied yet' });
    }
...
// Finance Manager GET all accepted items (ready for payment)
router.get('/accepted-items', async (req, res) => {
  try {
    // Fetch all requests containing items with inventoryStatus 'accepted'
    const requests = await Request.find({ 'items.inventoryStatus': 'accepted' });
...
router.patch('/pay/:requestId/:itemId', async (req, res) => {
  ...
  try {
    ...
    const isValidCode = /^[A-Z0-9]{10}$/.test(paymentCode) &&
    ...
    if (!isValidCode) {
      return res.status(400).json({
        success: false,
        message: 'Invalid payment code. It must contain 2 digits and 8 uppercase letters (total 10 characters).'
      });
    }
    ...
    const item = request.items.id(itemId);
    if (!item) {
      return res.status(404).json({ success: false, message: 'Item not found in request' });
    }

    if (item.inventoryStatus !== 'accepted' || item.paymentStatus !== 'unpaid') {
      return res.status(400).json({
        success: false,
...
router.get('/:requestId/:itemId/generate-receipt', async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        message: 'Receipt cannot be generated. Payment is either incomplete or missing details.'
      });
    }
    ...
    // Request Info
    doc.fontSize(12).text(`Receipt ID: RCPT-${item._id.toString().slice(-6).toUpperCase()}`);
    ...
    doc.text(`Payment Date: ${item.paymentDate ? new Date(item.paymentDate).toDateString() : 'N/A'}`);
    doc.text(`Generated On: ${new Date().toDateString()}`);
    doc.moveDown();

    // Footer
    doc.fontSize(10).text('Thank you for supplying quality materials to Corrugated Sheets Limited.');
    doc.text('We value your partnership and look forward to more business together.');
    doc.moveDown();
